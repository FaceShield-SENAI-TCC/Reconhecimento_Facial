...
def validate_face_image(face_img):
    ...
    if sharpness < 50:  # Reduzido o limite de nitidez
...
class FaceDetector:
    def __init__(self):
        ...
        self.detection_interval = 0.5  # Aumentado para dar mais tempo para detecção
        self.cached_faces = []

    def detect_faces(self, frame):
        """Detecta rostos no frame com múltiplos backends de fallback"""
        current_time = time.time()
        if current_time - self.last_detection_time < self.detection_interval:
            return self.cached_faces

        try:
            # Tentar diferentes backends em caso de falha, começando pelos mais robustos
            ...
            for backend in backends:
                try:
                    # Não redimensionar para melhor qualidade na detecção
                    ...
                    logger.info(f"✅ Detecção bem-sucedida com backend: {backend}")
                    break
                except Exception as e:
                    logger.warning(f"❌ Backend {backend} falhou: {e}")
                    continue

            if detected_faces is None:
                logger.warning("❌ Todos os backends de detecção falharam")
                self.cached_faces = []
                return []

            faces = []
            for face in detected_faces:
                if 'facial_area' in face and face['facial_area'] is not None:
                    ...
                    if (w >= MIN_FACE_SIZE[0] and h >= MIN_FACE_SIZE[1] and
                            face_roi.size > 0 and calculate_sharpness(face_roi) > 30):
                        faces.append((x, y, w, h))
                        logger.info(f"✅ Rosto detectado: {w}x{h} - Nitidez: {calculate_sharpness(face_roi):.1f}")

            self.cached_faces = faces
            self.last_detection_time = current_time
            logger.info(f"🔍 Detectados {len(faces)} rostos com backend: {backend_used}")
            return faces

        except Exception as e:
            logger.error(f"❌ Erro crítico na detecção: {str(e)}")
            return []
...
class FluidFaceCapture:
    def __init__(self, nome, sobrenome, turma, tipo, progress_callback=None, frame_callback=None):
        ...
        self.face_capture_interval = 1.0  # Aumentado intervalo entre capturas
    ...
    def _setup_camera(self):
        ...
        max_attempts = 5  # Aumentado número de tentativas
        for attempt in range(max_attempts):
            try:
                ...
                if not cap.isOpened():
                    ...
                    time.sleep(2)  # Aumentado tempo de espera
                    continue

                # Configurações de performance
                cap.set(cv2.CAP_PROP_FRAME_WIDTH, FRAME_WIDTH)
                cap.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_HEIGHT)
                cap.set(cv2.CAP_PROP_FPS, 15)  # Reduzido FPS para melhor estabilidade
                cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
                cap.set(cv2.CAP_PROP_AUTOFOCUS, 1)
                cap.set(cv2.CAP_PROP_FOCUS, 0)  # Desativa foco automático se disponível

                # Testar se a câmera funciona
                ret, test_frame = cap.read()
                if ret and test_frame is not None:
                    logger.info(f"✅ Câmera configurada na tentativa {attempt + 1}")
                    logger.info(f"📷 Resolução: {test_frame.shape[1]}x{test_frame.shape[0]}")
                    return cap
                else:
                    cap.release()
                    logger.warning(f"❌ Câmera não retornou frame na tentativa {attempt + 1}")
    ...
    def _cleanup_camera(self):
        ...
        try:
            ...
            for i in range(5):  # Tentar múltiplas câmeras
    ...
    def capture(self):
        ...
        try:
            ...
            no_face_timeout = 30  # Aumentado timeout para 30 segundos
            frame_count = 0
            ...
            while (self.running and
                ...
                # Detectar rostos a cada 5 frames para melhor performance
                if frame_count % 5 == 0:
                    faces = self.detector.detect_faces(frame)
                else:
                    faces = self.detector.cached_faces
                ...
                if face_detected:
                    ...
                    if (current_time - self.last_face_time) > self.face_capture_interval:
                        ...
                        if is_valid:
                            ...
                            logger.info(f"⚠️ Face detectada mas qualidade insuficiente: {validation_msg}")
                    else:
                    ...
                    if elapsed_no_face > no_face_timeout:
                        return False, "❌ Nenhum rosto detectado por 30 segundos. Verifique a iluminação e posicione seu rosto na câmera."
                ...
                time.sleep(0.05)  # Reduzido delay para melhor responsividade
